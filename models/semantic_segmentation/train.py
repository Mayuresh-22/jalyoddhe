...
def main(options): 
    ...
    if options['mode']=='train':
        ...
        for epoch in range(start, epochs+1):
            ...
            i_board = 0
            for (image, target) in tqdm(train_loader, desc=f"training {epoch}/{epochs}"):
                ...
                if isinstance(outputs, tuple):
                    logits, dsv4, dsv3, dsv2, dsv1 = outputs
                    
                    loss_main = criterion(logits, target)
                    
                    loss_dsv4 = criterion(dsv4, target)
                    loss_dsv3 = criterion(dsv3, target)
                    loss_dsv2 = criterion(dsv2, target)
                    loss_dsv1 = criterion(dsv1, target)
                    
                    loss = loss_main + 0.4 * (loss_dsv4 + loss_dsv3 + loss_dsv2 + loss_dsv1)
                else:
                    logits = outputs
            ...
            if epoch % eval_every == 0 or epoch==1:
                ...
                with torch.no_grad():
                    for (image, target) in tqdm(test_loader, desc=f"testing {epoch}/{epochs}"):
