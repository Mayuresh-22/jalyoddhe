...
def main(options):
    ...
    with torch.no_grad():
        ...
        if options['predict_masks']:
            ...
            path = os.path.join(data_path, 'data', 'patches')
            ROIs = np.genfromtxt(os.path.join(data_path, 'data', 'splits', 'test_X.txt'),dtype='str')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', default=os.path.join(up(os.path.abspath(__file__)), 'trained_models', '42', 'model.pth'), help='Path to Unet pytorch model')
